...
namespace Mirage
{
	...
	CUDA_CALLABLE inline void StratifiedSample1D(int c, int dx, ::Mirage::Random &rand, float &r1)
	{
		// map c onto stratum
		int x = c % dx;
	...
	CUDA_CALLABLE inline void StratifiedSample2D(int c, int dx, int dy, ::Mirage::Random &rand, float &r1, float &r2)
	{
		// map c onto stratum
		int x = c % dx;
		int y = (c / dx) % dy;
	...
	inline float ToiroidalDistanceSq(const float *v1, const float *v2, int dim)
	{
		...
		for (int i = 0; i < dim; ++i)
		{
			float delta = ::Mirage::Abs(v1[i] - v2[i]);

			if (delta > 0.5f)
				delta = 1.0f - delta;

			dSq += ::Mirage::Sqr(delta);
		}
	...
	void ProjectiveBlueNoiseSampling(int n, float *samples)
	{
		...
		float projectedRadiusSq = ::Mirage::Sqr(projectedRadius);
		...
		for (int i = 0; i < n; ++i)
		{
			...
			while (!valid)
			{
				...
				if (!valid)
				{
					...
					radius *= kReduction;
					radiusSq = ::Mirage::Sqr(radius);

					projectedRadius *= 0.999f;
					projectedRadiusSq = ::Mirage::Sqr(projectedRadius);
	...
	inline void ToiroidalShiftSample(float *shifted, const float *sample, int dim, ::Mirage::Random &rand)
	{
		for (int i = 0; i < dim; ++i)
		{
...
#define USE_BLUE_NOISE 0
	...
	CUDA_CALLABLE inline void Sample1D(::Mirage::Random &rand, float &u1)
	{
#if USE_RANDOM
	...
	CUDA_CALLABLE inline void Sample2D(::Mirage::Random &rand, float &u1, float &u2)
	{
#if USE_RANDOM
